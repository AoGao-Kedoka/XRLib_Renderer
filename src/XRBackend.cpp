...
void XRBackend::CreateXrInstance()
{
    ...
    if ( xrEnumerateInstanceExtensionProperties(nullptr, 0, &extensionCount, nullptr)) {
    ...
    strcpy(instanceCreateInfo.applicationInfo.applicationName, info.applicationName.c_str());
    instanceCreateInfo.applicationInfo.applicationVersion = info.version;
    strcpy(instanceCreateInfo.applicationInfo.engineName, info.applicationName.c_str());
    ...
    LOGGER(LOGGER::INFO) << "Using XR Runtime: " << instanceProperties.runtimeName;
}
