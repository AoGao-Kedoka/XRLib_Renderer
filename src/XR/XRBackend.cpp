...
void XrBackend::Prepare() {
    ...
    // prepare view projection matrices
    viewMatrices.resize(viewCount);
...
void XrBackend::CreateXrSwapchain() {
    ...
    if (xrEnumerateViewConfigurationViews(
            xrCore->GetXRInstance(), xrCore->GetSystemID(),
            xrCore->GetXrViewConfigurationType(), 0, &viewCount,
            nullptr) != XR_SUCCESS) {
...
XrResult XrBackend::EndFrame(uint32_t& imageIndex) {
    ...
    if ((result = xrReleaseSwapchainImage(xrCore->GetXrSwapchain(),
        ...
        XR_SUCCESS) {
        LOGGER(LOGGER::WARNING)
            << "Failed to release swapchain image" << result;
    }
...
void XrBackend::BeginSession() {
    ...
    sessionBeginInfo.primaryViewConfigurationType =
        xrCore->GetXrViewConfigurationType();
        ...
        switch (eventData.type) {
            ...
            case XR_TYPE_EVENT_DATA_SESSION_STATE_CHANGED: {
                ...
                if (sessionStateChanged->session != xrCore->GetXRSession()) {
                    LOGGER(LOGGER::WARNING)
                        << "XrEventDatasessionStateChanged for unknow session";
                    break;
                }
...
void XrBackend::UpdateViews() {
    ...
    if ((result = xrLocateViews(xrCore->GetXRSession(), &viewLocateInfo,
                                &xrCore->GetXrViewState(),
                                xrCore->GetXrViews().size(), &viewCount,
                                xrCore->GetXrViews().data())) != XR_SUCCESS) {
    ...
    for (size_t i = 0; i < viewCount; ++i) {
        XrCompositionLayerProjectionView& cpLayerProjectionView =
            xrCore->GetCompositionLayerProjectionViews()[i];
