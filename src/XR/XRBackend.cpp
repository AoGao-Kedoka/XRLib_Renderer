...
void XRBackend::CreateXrInstance() {
  ...
  std::memcpy(instanceCreateInfo.applicationInfo.applicationName,
              info->applicationName.c_str(), XR_MAX_APPLICATION_NAME_SIZE - 1);
  ...
  if (info->validationLayer) {
    ...
    if (xrCreateDebugUtilsMessengerEXT(xrCore->GetXRInstance(),
...
void XRBackend::CreateXrSwapchain() {
  ...
  if ((result = xrCreateSwapchain(xrCore->GetXRSession(), &swapchainCreateInfo,
...
void XRBackend::PollEvents() {
  ...
  while (XrPoolEvent()) {
    ...
    case XR_TYPE_EVENT_DATA_EVENTS_LOST: {
      ...
      LOGGER(LOGGER::INFO) << "OpenXR events lost: "
...
void XRBackend::LogOpenXRSystemProperties() const {
  ...
  if (xrGetSystemProperties(xrCore->GetXRInstance(), xrCore->GetSystemID(),
    ...
    LOGGER(LOGGER::INFO) << "    Max Layer Count: "
