...
VkImageView& Image::GetImageView() {
    if (imageView == VK_NULL_HANDLE) {
        ...
        if (vkCreateImageView(core->GetRenderDevice(), &imageViewInfo, nullptr,
...
VkSampler& Image::GetSampler() {
    if (sampler == VK_NULL_HANDLE) {
        ...
        if (vkCreateSampler(core->GetRenderDevice(), &samplerInfo, nullptr,
                            &sampler) != VK_SUCCESS) {
...
void Image::CreateImage(uint32_t width, uint32_t height, VkFormat format,
    ...
    if (vkCreateImage(core->GetRenderDevice(), &imageInfo, nullptr, &image) !=
        VK_SUCCESS) {
        throw std::runtime_error("failed to create image!");
    }

    VkMemoryRequirements memRequirements;
    vkGetImageMemoryRequirements(core->GetRenderDevice(), image,
                                 &memRequirements);
