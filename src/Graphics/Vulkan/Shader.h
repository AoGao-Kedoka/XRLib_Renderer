...
class Shader {
   public:
    ...
    Shader(std::shared_ptr<VkCore> core, const std::filesystem::path& file_path, ShaderStage stage, bool stereo);
    ~Shader();

    Shader(Shader&& other) noexcept
        : core(std::exchange(other.core, nullptr)), stage(std::exchange(other.stage, static_cast<ShaderStage>(0))),
